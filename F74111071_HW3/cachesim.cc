...
cache_sim_t::cache_sim_t(const cache_sim_t& rhs)
    : sets(rhs.sets), ways(rhs.ways), linesz(rhs.linesz), idx_shift(rhs.idx_shift), name(rhs.name), log(false) {
    tags = new uint64_t[sets * ways];
...
uint64_t* cache_sim_t::check_tag(uint64_t addr) {
    ...
    size_t tag = (addr >> idx_shift) | VALID;  // VALID = 1ULL << 63

    for (size_t i = 0; i < ways; i++)
        if (tag == (tags[idx * ways + i] & ~DIRTY))  // DIRTY = 1ULL << 62, temporarily set dirty bit
...
uint64_t cache_sim_t::victimize(uint64_t addr) {
    size_t idx = (addr >> idx_shift) & (sets - 1);
    size_t way = way_idx[idx];
    uint64_t victim = tags[idx * ways + way];
    tags[idx * ways + way] = (addr >> idx_shift) | VALID;  // replace old tag with new tag
    way_idx[idx]++;                                        // next way index to replace
    way_idx[idx] %= ways;
    return victim;
}
...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store) {
    ...
    if (likely(hit_way != NULL)) {  // if hit && store
        if (store)
            *hit_way |= DIRTY;  // set dirty bit = 1
        return;
    }
    ...
    // if store, set dirty bit of new data = 1
    if (store)
...
uint64_t* fa_cache_sim_t::check_tag(uint64_t addr) {
    ...
    for (size_t i = 0; i < ways; i++)
        if (tag == (tags[i] & ~DIRTY))  // DIRTY = 1ULL << 62, temporarily set dirty bit
            return &tags[i];
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr) {
    size_t way = way_idx[0];
    uint64_t old_tag = tags[way];
    tags[way] = (addr >> idx_shift) | VALID;  // replace old tag with new tag
    way_idx[0]++;                             // next way index to replace
    way_idx[0] %= ways;
    return old_tag;
}
